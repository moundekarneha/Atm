...
namespace Atm
{
    public partial class Lic : Form
    {
            ...
            try
            {
                if (CheckEnterText())
                {
                    ...
                    if (balance >= int.Parse(txtAmount.Text))
                    {
                        ...
                        globalClass.RunNonQuery("insert into Transac values(" + 503 + ",'" + DateTime.Now.ToShortDateString() + "'," + txtAmount.Text + ",0,0)");
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            if (CheckEnterText())
            {
                if (globalClass.CheckBill(txtBillNum.Text, "Lic"))
                {
                    int billAmount = globalClass.GetBillAmnt("Lic");
